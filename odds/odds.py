...
def detect(race_id):
    ...
    shutuba_df = pd.read_html(shutuba_url)[0]
    shutuba_df = shutuba_df.T.reset_index(level=0, drop=True).T
    shutuba_df = shutuba_df[['馬番', '馬名']]
    shutuba_df = shutuba_df.set_axis(['umaban', 'umamei'], axis=1)

    driver.get(url)
    time.sleep(0.1)
    title = driver.title[:-26]
    t = driver.find_element_by_class_name('RaceData01').text
    title = title + ' / ' + t

    # 単勝・複勝
    tanfuku = driver.find_element_by_id('Ninki')
    umabans = tanfuku.find_elements_by_tag_name('tr')[1:]
    tansho = []
    umaban_list = []
    for umaban in umabans:
        if '取消' in umaban.text or '除外' in umaban.text:
            umaban_list.append(int(umaban.find_elements_by_tag_name('td')[2].text))
            continue
        umaban_list.append(int(umaban.find_element_by_tag_name('span').text))
    ...
    # 馬券とそのオッズを取得し、スコアを計算
    combis = driver.find_elements_by_class_name('Combi01')
    odds = driver.find_elements_by_class_name('Name_Odds')
    ...
    # ドロップダウンを取得
    dropdown = driver.find_element_by_id('ninki_select')
    n_dropdown = len(dropdown.find_elements_by_tag_name('option'))
    select = Select(dropdown)

    if n_dropdown>3:
        for i in range(2, n_dropdown-2, 1):
            # 3ページ目で中断
            if i == 3:
                break
